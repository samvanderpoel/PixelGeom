...
s = 0.25   # param to tune granularity/resolution of frames
imgs = [] # where frames are stored

# Replace shape-generating function here:
testshp = dodecahedron(name = 'test', center = (s*499, s*499, s*100), rad = s*400, shade = 1)
mygrid = grid(shapes = [testshp], dim = (round(s*1000), round(s*1000)))
mygrid.paint_canvas('gradient')
...
for idx, i in enumerate(np.linspace(0.1, 10, num = frames)):
    ...
    sys.stdout.flush()
    mygrid.rotate_shape3d(name = 'test', axis = [0.5, math.cos(i), math.sin(i)], angle = interval * math.pi/5)
    mygrid.rotate_shape3d(name = 'test', axis = [1, 0.25, 0.75], angle = interval * math.pi/5)
    mygrid.draw_shapes()
    cmap = rgb_to_cmap(colors = [[107,142,35], [123,104,238], [210,105,30]], penlow = None, penhigh = [160,0,0])
...
video=cv2.VideoWriter('tests/test-output/test-vid.mp4', fourcc, 12, (1000, 1000))
n = len(imgs)
m = len(str(n))
